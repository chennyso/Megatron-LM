...
CAMPAIGN_NAME="${CAMPAIGN_NAME:-q14_crossnode_pp8_tp2_$(date +%Y%m%d)}"
...
run_case() {
  ...
  if [[ -n "${layout_mode}" ]]; then
    layout="$(${PYTHON_BIN} perf_pp_vpp/scripts/13_generate_topology_aware_layout.py \
      --pp 8 --vpp "${vpp}" --num-layers 40 --mode "${layout_mode}")"
  fi
  ...
  perf_pp_vpp/scripts/03_run_megatron_experiment.sh \
    --experiment-yaml perf_pp_vpp/configs/experiments/phase1_qwen14b_crossnode_pp8_tp2.yaml \
...
run_case q14_pp8_dp1_tp2_vpp2 3301 uniform_vpp2 uniform 2 || true
run_case q14_pp8_dp1_tp2_vpp2 3302 boundary_dilated_vpp2 boundary_dilated 2 || true
run_case q14_pp8_dp1_tp2_vpp4 3303 uniform_vpp4 uniform 4 || true
run_case q14_pp8_dp1_tp2_vpp4 3304 boundary_dilated_vpp4 boundary_dilated 4 || true
