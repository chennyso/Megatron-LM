...
ROOT="${RUN_ROOT:-/workspace/runs/g5-8gpu-strategy-research-20260829/topology_permutation_pp4_$(date -u +%Y%m%dT%H%M%SZ)}"
CODE="${CODE_DIR:-/workspace/code/Megatron-LM}"
mkdir -p "$ROOT"

export NCCL_P2P_DISABLE=1 NCCL_IB_DISABLE=1 NCCL_SOCKET_IFNAME=eth0
export CUDA_DEVICE_MAX_CONNECTIONS=1 OMP_NUM_THREADS=8
...
BASE=(
  ...
  --micro-batch-size 1 --global-batch-size 8 --bf16 --use-mcore-models
  --use-distributed-optimizer --position-embedding-type rope
  --rotary-percent 1.0 --rotary-base 10000
  ...
  --train-iters 45 --eval-iters 0 --eval-interval 100000
  --lr 1e-6 --min-lr 1e-7 --lr-decay-style constant
  --log-interval 1 --seed 20260829 --pipeline-schedule interleaved
...
run_case() {
  ...
  CUDA_VISIBLE_DEVICES="$order" timeout --signal=TERM --kill-after=45s 1200 \
    python3 -m torch.distributed.run --standalone --nproc-per-node=8 \
    "$CODE/pretrain_gpt.py" "${BASE[@]}" "$@" > "$out/train.log" 2>&1
  local rc=$?
  printf '%s\t%s\t%s\n' "$name" "$rc" "$(date -u +%Y-%m-%dT%H:%M:%SZ)" >> "$ROOT/status.tsv"
  grep 'elapsed time per iteration' "$out/train.log" | tail -20 > "$out/steady.tsv" || true
  grep -E 'Traceback|RuntimeError|ValueError|AssertionError|NCCL' "$out/train.log" | tail -60 > "$out/errors.txt" || true
}

# Same PP4/TP2/VPP4 plan, different physical-to-logical rank mappings.
# default: each TP pair follows the original rank order.
run_case default_01234567 0,1,2,3,4,5,6,7
# numa_grouped: keeps the three same-NUMA pairs while moving the isolated GPU7 boundary.
run_case numa_grouped 0,1,4,5,2,3,6,7
# cross_pair_control: deliberately creates four cross-NUMA TP pairs.
